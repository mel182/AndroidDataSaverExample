...
suspend fun <T : Any> Call<T>.awaitNonNullable(): T {
    ...
    return suspendCancellableCoroutine { continuation ->
        ...
        enqueue(object : Callback<T?> {
            override fun onResponse(call: Call<T?>?, response: Response<T?>) {
                ...
                Log.i("TAG55","on response L66: ${response.body}")
            ...
            override fun onFailure(call: Call<T?>?, t: Throwable?) {
                Log.i("TAG55","on failure L78: ${t?.message}")
                continuation.resumeWithException(t ?: java.lang.Exception("request failed for unknown reason"))
            }
        })
    }
...
suspend fun Call<Unit>.await() {
    ...
    (this as Call<Unit?>).awaitNullable()
}
...
suspend fun <T> Call<T>.awaitResponse(): Response<T> {
    return suspendCancellableCoroutine { continuation ->
        ...
        enqueue(object : Callback<T> {
            ...
            override fun onResponse(call: Call<T>?, response: Response<T>) {
                Log.i("TAG55","on response L98: ${response.body}")
            ...
            override fun onFailure(call: Call<T>?, t: Throwable?) {
                Log.i("TAG55","on failure L105: ${t?.message}")
                continuation.resumeWithException(t ?: java.lang.Exception("request failed for unknown reason"))
            }
        })
    }
