...
class Utils {
    ...
    companion object {
        ...
        fun parameterError(
            ...
            Log.i("TAG55","parameterError function called!")

            val result = methodError(
                method,
                "$message (parameter # ${(p + 1)})",
                *args
            )
        ...
        fun hasUnresolvableType(type: Type?): Boolean {
            ...
            val className = if (type == null) "null" else type::class.java.name
            requireNotNull(type) { "Expected a Class, ParameterizedType, or GenericArrayType, but <$type> is of type $className" }
